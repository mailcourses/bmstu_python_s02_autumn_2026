# Домашнее задание #01 (тестирование)

### 1. Функция оценки сообщения
Реализовать функцию `predict_message_mood`, которая принимает на вход строку `message` и пороги хорошести (от 0 до 1).
Функция возвращает:
- "неуд", если предсказание модели меньше `bad_threshold`;
- "отл", если предсказание модели больше `good_threshold`;
- "норм" в остальных валидных случаях;
- различные исключения для случаев некорректных порогов или значения предсказания;
...
assert Exception predict_message_mood("Что-то не так", 1, -1)
```
